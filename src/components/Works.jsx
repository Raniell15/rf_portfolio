...
export const Works = () => {
    ...
    useEffect(() => {
      ...
      const handleScroll = () => {
        ...
        if (category === 'all') {
            setIsScrolled(scrollPosition > 3600);
        } else if (category === 'software') {
            setIsScrolled(scrollPosition > 3000);
        } else if (category === 'hardware') {
            setIsScrolled(scrollPosition > 2500);
        } else {
            setIsScrolled(scrollPosition > 2200);
        }
  ...
  return (
    <section id='work' className="flex flex-col items-center justify-center min-h-screen space-y-10">
        <h2 className={`text-2p text-3xl opacity-0 md:text-4xl  ${isScrolled ? "animate-fade-in" : ""}`}>My <span className='text-app-primary'>Works</span></h2>
        <ul className="timeline timeline-vertical">
            {projects.map((project) => (
                <li key={project.id}>
                    {project.id != 1 && <hr/>}
                    <div  className={`timeline-box overflow-hidden space-y-4 max-w-100 md:max-w-150 card-hover opacity-0 ${isScrolled ? `animate-fade-in-delay-${project.id <= 5 ? `${project.id}` : '5'}` : ""} ${project.id % 2 === 0 ? "timeline-end" : "timeline-start"}`} style={{padding:0}}>

                        <WorkCarousel image={project.image}/>
                        <div className='flex text-silk gap-3 flex-wrap p-4 '>
                            {project.tags.map((tag, index) => (
                                <h2 key={index} className='rounded-full border-solid border-1 border-app-primary p-2'>
                                    {tag}
                                </h2>
                            ))}
                        </div>
                        <h2 className='text-2p text-2xl text-app-primary mt-5'>{project.title}</h2>
                        <p className='text-silk text-sm px-4'>{project.description}</p>
                        <div className='flex p-4 space-x-2'>
                            <a href={project.demoUrl}><Link/></a>
